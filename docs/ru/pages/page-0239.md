...
Код в гитхаб-репозитории, который в комплекте, дорабатывает эту тему под sized-генераторы (sized generators) — чтоб не просто так болталось.
...
Генерим `Gen[List[Int]]` через `choose(-100, 100).listOfN(choose(0, 20))`. Потом мапим по этому генератору и редуцируем список в один `Par[Unit]` с помощью `fork` и `map2` — чистый комбинаторный дзен. А чтоб не ебаться повторно, выносим generic фичу на списках под `parTraverse` и лепим на неё наш генератор, как конструктор Лего в руках FP-маньяка:
