...
опциональных значений; мы просто после всего этого дерьма подтянули его в контекст ``Option`` постфактум. Этот трюк прокатывает с любой функцией, блядь. Давай глянем другой примерок, чтоб дошло. Представь, пилю логику для сайта автостраховки – там форма, юзеры стучат по кнопке "дай котировку онлайн мгновенно". Хотим распарсить эту хуйню из формы и в итоге дёрнуть нашу `rate`:
...
Мы хотим вызвать эту функцию, но юзер шлёт возраст и количество лихачеств в веб-форме – прилетает голимая строковая поебень, которую надо (попробовать) в инты перекинуть. Парсинг может наебнуться к херам; дана строка ``s``, пытаемся запарсить в ``Int`` через ``s.toInt``, а она кидает ``NumberFormatException``, если строка не интовая хуйня:
