...
`drop` лепим рекурсивной функцией, которая откусывает по одному элементу (uncons) и рекурсит, пока счётчик дропа не скатится до нуля — как резал бы логи из лога, пока не надоест:
...
`takeWhile` похож: сначала uncons элемент, потом думаем, рекурсить ли. Если элемент прошёл предикат — вываливаем его и дёргаем `takeWhile` на хвосте. Нет — возвращаем этот элемент вперёд хвоста как pull. А если источник на нуле — сливаем накопленное и валим, без драм:
