...
Вместо этого давай зафигачим opaque type alias (непрозрачный псевдоним типа) для `Pull[O, Unit]` и привяжем к нему map-then-concat поведение — чтоб не париться с этими инстансами. Назовём эту прелесть `Stream`, потому что фиксируем конец pull'а на `Unit`, и весь фокус уходит на выходные значения, как в нормальном стриме, где хвост виляет:
