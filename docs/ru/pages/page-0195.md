...
> Актор юзает ExecutorService, чтоб обрабатывать сообщения по приходу, так что здесь мы его и создаём.
...
> Это простейший актор, который просто эхоит строковые сообщения. Обратите внимание, мы пихаем ему s — ExecutorService для обработки сообщений.
...
> Отправляет “goodbye” актору. Тот в ответ кидает таску в свой ExecutorService на обработку.
...
Теперь можем заимплементить `map2` через `Actor`, который соберёт результаты с обоих аргументов. Код прямолинейный, race conditions хуй там плавал — мы ж знаем, что `Actor` обрабатывает ровно одно сообщение за раз, как строгий бармен в очереди.
...
> Используются две mutable var для хранения двух результатов — да, mutable, но в этом контексте как нож в руках хирурга, а не долбоёба.
