...
Реализация ``IndexedSeq`` делает то же самое, но ещё переопределяет ``foldMap``, используя ``foldMapV``, чтоб результат вычислить по-умному и эффективно, без этой ленивой хуйни, которая жрёт стек как бешеная:
...
Реализация ``LazyList`` похожа на ``List``, просто делегирует ``foldLeft`` и ``foldRight`` подложенному типу данных — классический прокси-паттерн, чтоб не дублировать код, как лохи:
