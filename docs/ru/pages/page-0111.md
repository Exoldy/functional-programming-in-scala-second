...
Эта функция — как швейцарский нож, настолько общая, что её можно запилить прямо в companion object (объект-супруг) к ``Either``, без привязки к какому-то одному узкому кейсу, который завтра забудется.
...
Заметим, что с этими дефами ``Either`` теперь пляшет в for-comprehension'ах (for/yield-выражениях) как родной (вспомните, это ж сахар над ``flatMap``, ``map`` и всей этой тусовкой). Вот, к примеру:
