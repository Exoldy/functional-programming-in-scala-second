...
---
# Страница 0203
[<- Страница 0202](./page-0202) | [Индекс страниц](./) | [Страница 0204 ->](./page-0204)
...
В итоге остаётся голая функция, которая берёт `A` и выдаёт `Par` — та кидает джоб в экзекьютор, и когда этот джоб рванёт на ОС-треде, он честно посчитает `f(a)`. Классика, без подвохов.
...
Но можно и получше провернуть, как в `sums` — технику ту же самую. Разделим компьютацию на две половины, каждую параллельно запустим. Поскольку нужен быстрый рандом-акцесс к элементам коллекции (а не linearnый треш), сначала версию на `IndexedSeq[Par[A]]` напишем:
