...
Заметим, что `traverse` похож на `map`, но впаривает лишний type constructor `G`. А если подобрать для `G` тип, чтоб этот constructor испарился, как пар от кофе на код-ревью? Вспомним слегка подкрученную версию типа `Id` из прошлой главы, только теперь как type alias, а не case class — проще пареной репы:
...
Короче, траверс с identity type constructor'ом вырождается в чистый map — как суперкар в режиме экономии бензина! Полная имплементация вот такая:
