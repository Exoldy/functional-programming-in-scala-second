...
Чтобы завести `map2`, матчим по двум валидированным значениям, как по двум пьяным корешам на код-ревью. Оба в теме — апплайим поданную ф-ю и оборачиваем результат в `Valid`, чтоб не потерялся. Оба в говне — склеиваем ошибки через данную `Monoid[E]`, типа "давайте, братаны, объединим ваши фейлы в эпик фейл"; а если один в говне — возвращаем его, без сантиментов, чтоб не плодить лишних мутантов:
...
Нам надо доказать, что все аппликативные законы держатся, когда мы лепим `map2` на базе `flatMap`: левая идентичность, правая, ассоциативность и натурализм. Берем это определение `map2`, чтоб не выдумывать велосипед:
...
Вот пруф левой идентичности, шаг за шагом, как в пьяной тусовке разбираем, почему код не компилится:
