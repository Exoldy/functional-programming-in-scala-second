...
Для `combine` берём первую половину входных пар, моноидим её с `ma`, вторую — с `mb`, и возвращаем свежую пару, как из коробки. А `empty` — это просто спарка пустышек от `ma` и `mb`, классический zero для пары:
