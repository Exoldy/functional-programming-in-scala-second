...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run RU QC in parallel with subprocess workers.")
...
def collect_targets(include_completed: bool, limit: int | None) -> list[str]:
    ...
    if include_completed:
        ...
        targets = [name for name in all_pages if name not in completed]
...
def main() -> None:
    ...
    prior_failures = load_json(FAILURES_PATH, [])
    failure_map = {item["file"]: item for item in prior_failures if "file" in item}
            ...
            for future in finished:
                ...
                if ok:
                    ...
                    if attempts[result_page] <= args.retries:
                        ...
                        print(
                            f"retry {result_page} attempt={attempts[result_page]} reason={error[:160]}",
                            flush=True,
                        )
                    else:
                        with failure_lock:
                            ...
                            save_json(
                                FAILURES_PATH,
                                sorted(failure_map.values(), key=lambda item: item["file"]),
                            )
                        done_fail += 1
